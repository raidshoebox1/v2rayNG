...
- **Network secret** is stored in `EncryptedSharedPreferences` (AES-256-GCM via `androidx.security:security-crypto:1.1.0-alpha06`, which is the version that ships the `MasterKey` API) in a separate file (`easytier_secret`), not in the default plaintext prefs.

...
//    }
    ...
    private fun configureNetworkSettings(builder: Builder) {
        ...
        if (bypassLan) {
            ...
            // to reach Xray-core's EasyTier SOCKS5 outbound. We add only the subnets
            // EasyTier actually owns (configured virtual-IP subnet + discovered mesh
            // CIDRs), NOT the whole RFC1918 space, so the device's real LAN is left
            // untouched when EasyTier is not claiming those routes.
            try {
                val etConfig = EasyTierSettingsManager.getEasyTierConfig(this)
                if (etConfig != null && etConfig.enabled) {
                    val meshCidrs = EasyTierPlugin.getEffectiveRoutingCidrs(this)

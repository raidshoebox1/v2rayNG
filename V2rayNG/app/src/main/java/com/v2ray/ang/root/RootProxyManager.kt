...
object RootProxyManager {
    ...
    private fun buildMangleMarking(
        ...
        return buildString {
            ...
            // the tun even though they fall inside the private ranges above. We only
            // re-mark the subnets EasyTier actually owns (configured virtual-IP subnet +
            // discovered mesh CIDRs), NOT the whole RFC1918 space, so the real LAN stays
            // direct when EasyTier is not claiming those routes.
            if (cmd == "iptables") {
                try {
                    val etConfig = EasyTierSettingsManager.getEasyTierConfig(context)
                    if (etConfig != null && etConfig.enabled) {
                        val meshCidrs = EasyTierPlugin.getEffectiveRoutingCidrs(context)
                        ...
                        LogUtil.d(AppConfig.TAG, "EasyTier: re-marked ${meshCidrs.size} mesh CIDRs in root mangle chain (v4)")
                    }
                } catch (e: Throwable) {
                    LogUtil.w(AppConfig.TAG, "EasyTier: failed to re-mark mesh CIDRs in root mode", e)
                }
            }
            // IPv6 mesh CIDRs: re-mark so they route into the tun instead of being
            // bypassed by the v6 RETURN rules above.
            if (cmd == "ip6tables") {
                try {
                    val etConfig = EasyTierSettingsManager.getEasyTierConfig(context)
                    if (etConfig != null && etConfig.enabled) {
                        val meshCidrs = EasyTierPlugin.getEffectiveRoutingCidrs(context)
                            .filter { it.contains(":") } // IPv6 only for ip6tables
                        meshCidrs.forEach { cidr ->
                            appendLine("$cmd -t mangle -A $CHAIN -d $cidr -j MARK --set-xmark $MARK")
                        }
                        LogUtil.d(AppConfig.TAG, "EasyTier: re-marked ${meshCidrs.size} mesh CIDRs in root mangle chain (v6)")
                    }
                } catch (e: Throwable) {
                    LogUtil.w(AppConfig.TAG, "EasyTier: failed to re-mark v6 mesh CIDRs in root mode", e)
                }
            }
            if (allowMode) {
    ...
    private fun buildLanShareSetup(context: Context, captureDeviceTraffic: Boolean, ipv6: Boolean): String {
        ...
        return buildString {
            ...
            // Only the subnets EasyTier actually owns are re-routed, so the real LAN stays
            // direct via `main` unless EasyTier claims it.
            try {
                val etConfig = EasyTierSettingsManager.getEasyTierConfig(context)
                if (etConfig != null && etConfig.enabled) {
                    val meshCidrs = EasyTierPlugin.getEffectiveRoutingCidrs(context)
                        .filter { !it.contains(":") } // ip rule "to" is IPv4 for the v4 section
